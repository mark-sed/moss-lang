...
static void report_ir_exception(ir::IR *err, ustring error_name) {
    if (auto exc = dyn_cast<ir::Raise>(err)) {
        ...
        errs << error_name << ": " << err_msg->get_value();
...
int main(int argc, const char *argv[]) {
    ...
    if (!input_is_msb) {
        ...
        if (auto exc = dyn_cast<ir::Raise>(main_mod)) {
            report_ir_exception(exc, "SyntaxError");
        }
